...
folder = '/Users/pr-mbausr/Library/CloudStorage/OneDrive-Personal/market data/public companies/json'

def alphavantage_company_reports(symbols, report_options):
    """ 
    args:
    company is ticker
    ...
    for symbol in symbols:
        for report in report_options:
            ...
            with open(f'{folder}/{symbol}/{symbol}-{report}-av.json', 'w') as outfile:
...
def main():
    ...
    if report_selection == [0]:
        alphavantage_company_reports(ticker, list(report_options.values()))
    else:
        selected_keys = [list(report_options.keys())[choice - 1] for choice in report_selection]
        alphavantage_company_reports(ticker, selected_keys)
...
3. if file exists append update
4. maybe - name in title
5. Script to run daily w/ some company
  - call from yaml or other file
"""
...
some_companies = 'CDAY GOOG IT PAYX WDAY FDX UPS AMZN NOW FLN MSFT GB'